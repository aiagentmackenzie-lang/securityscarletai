...
| **V0.5 "Fleet & Scale"** | Group C: fleet identity + raw-line fleet ingest + (next) TimescaleDB + deployment story | fleet enrollments -> fleet shipper -> Timescale hypertables -> small-fleet overlay | 🟡 **V0.5a + V0.5b DELIVERED 2026-09-11 (branch feat/v0.5a-fleet-enrollment, push pending)**: per-host enrollment (sha256-at-rest tokens, host-bound at ingest, spoof -> 403 + audited, revocation immediate) + POST /ingest/osquery (server-side parsing = one ECS truth, same writer + coalesced correlation) + standalone stdlib fleet_shipper.py (checkpoint/rotation/partial-line discipline, at-least-once delivery, 401/403 fatal). TimescaleDB (image swap + PK restructure on the standing volume) = HITL, next. Deployment overlay (V0.5d) queued. |
