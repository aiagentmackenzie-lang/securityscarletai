...
def make_event(
    ...
    if event_action:
        raw["event_action"] = event_action
    if user_name:
        raw["user_name"] = user_name
    if process_name:
        raw["process_name"] = process_name
    if process_cmdline:
        raw["process_cmdline"] = process_cmdline
    if process_path:
        raw["process_path"] = process_path
    if process_pid:
        raw["process_pid"] = process_pid
    if source_ip:
        raw["source_ip"] = source_ip
    if destination_ip:
        raw["destination_ip"] = destination_ip
    if destination_port:
        raw["destination_port"] = destination_port
    if file_path:
...
def main():
    ...
    for i in range(0, len(events), 50):
        batch = events[i : i + 50]
        resp = httpx.post(
            API_BASE, json=batch, headers={"Authorization": f"Bearer {TOKEN}"}, timeout=10
        )
