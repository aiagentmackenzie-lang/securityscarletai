...
## ADDENDUM — fix wave (Raphael's go: "proceed with all fixes")

| FT | Disposition | Receipt |
|---|---|---|
| FT-001 | **SHIPPED** (NeuralStrike branch `fix/fleet-ft001-bench-verdicts` @ 1d86480): auth rejections → `auth_error` non-measurement class; excluded from firewall_caught/catch_rate + purple-report caught status; new `firewall_auth_errors` counter + loud bench warning + all-auth run raises instead of writing a receipt. Pins FAIL-proven (4 fail on old code). NS L2: 918 collected / 917 passed / 1 pre-existing env-dependent failure (verified failing on main with the local .env overriding the asserted default — passes without .env) / coverage 90.51%. | pins + suite |
| FT-002 | **RESOLVED BY DESIGN** — no per-run keys needed: NG-side attribution = the key's bound tenant (demo) now flows to ScarletAI via the bridge (user_name=demo); NS-side run attribution = run-stamped hosts + source=neuralstrike (already live). Per-run tenant keys remain an option if NG-side run attribution is ever wanted (HITL env change). | bridge rows + NS telemetry |
| FT-003 | **WIRED + LIVE-VERIFIED** (NeuralGuard branch `fix/fleet-wave-f-ng` @ 32961b1 + be638b5): NEURALGUARD_SIEM_* in .env (enabled, host.docker.internal:8000/api/v1/ingest, scoped INGEST_BEARER_TOKEN) + override passthrough committed; NG recreated; `siem_router_registered` sinks=[scarletai] in boot logs; live probe → ScarletAI rows **parent verdict_block (critical) + ai companion, user_name=demo**, host_name=neuralguard-macmini (FT-003 ops line); purple-report re-run with the receipt: `scarlet.firewall_events = {verdict_block: 8, ai_prompt_injection: 7}` (15 events) + **firewall_alerts: 2** — the SIEM now fires alerts off NG verdicts. Pre-wire connectivity verified from the container (200). | DB rows + report JSON |
| FT-004 | **SHIPPED** (same NG branch @ 84f6aea): `judge_ollama_think` (bool\|None, default None = NO think field sent — fail-closed, payload byte-identical). NEURALGUARD_SCANNER_JUDGE_OLLAMA_THINK=false unlocks thinking-capable judges (nemotron). Pins FAIL-proven (2 new-behavior fail on old code; 2 guards). NG L2: **1,284 passed / 8 skipped / 0 failed**. Lands in the running container at the next natural NG rebuild (knob is unset — no behavior change live). | pins + suite |
| FT-005 | **SHIPPED** (.env edit — gitignored, no commit): duplicate NEURALGUARD_RATELIMIT_BACKEND removed (redis kept; container-verified). Running container untouched by this item. | .env grep = 1 |
| FT-006 | **SHIPPED** (ScarletAI branch `fix/ft007-benchmark-usage` @ 9301fb8): keep-alive perturbation ops note in the harness docstring. | docstring |
| FT-007 | **SHIPPED** (same branch @ edd5317): benchmark calls → ai_usage (user=None system actor, ai.benchmark_* endpoints, tokens/latency/source/fallback persisted, fail-soft). Pins FAIL-proven (2). ScarletAI L2: **2,606 passed / 0 failed / exit 0 / 88.32%** (2,603 floor + 3 pins). | pins + suite |
| FT-008 | No fix (observe-only; not reproduced). | — |
| FT-009 | NOT actioned — a limit raise vs leak hunt stays Raphael's explicit call (memory flat through L1–L3 this session; no leak evidence at these loads). | stats receipts |

Secret scans on all three branch diffs (manual rg; sentinel not installed): clean.

**Final states:** ScarletAI main = origin/main = 342bc44 (untouched; branches `fix/ft007-benchmark-usage` @ 9301fb8 + `runs/fleet-wave-f` @ this commit, local-only) · NeuralGuard main = 95c827a = origin (branch `fix/fleet-wave-f-ng` @ be638b5, 4 commits, local-only) · NeuralStrike main = 2664aa2 = origin (branch `fix/fleet-ft001-bench-verdicts` @ 1d86480, local-only). **No push, no merge anywhere without Raphael's go.**
